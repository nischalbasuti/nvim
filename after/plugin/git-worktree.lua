...
-- Place in: ~/.config/nvim/after/plugin/git-worktree.lua
...
local function branch_exists(branch_name, cwd)
  ...
  -- Check if it's a valid ref (handles origin/branch format)
...
function M.list_worktrees()
  ...
  -- Don't forget the last one if no trailing newline
...
function M.create_worktree(worktree_name, branch_name, source_branch)
  ...
  -- Build worktree path as sibling to current worktree directory
  ...
  if not branch_exists(branch_name, main_worktree) then
    ...
    -- Branch exists, proceed directly
    M._create_worktree_impl(worktree_path, branch_name, main_worktree, original_branch)
  end
end
...
function M.setup(opts)
  ...
  -- vim.keymap.set("n", "<leader>wc", M.create_worktree_interactive, { desc = "Create worktree" })
  -- vim.keymap.set("n", "<leader>wr", M.remove_worktree_interactive, { desc = "Remove worktree" })
  -- vim.keymap.set("n", "<leader>wl", M.show_worktrees, { desc = "List worktrees" })
  -- vim.keymap.set("n", "<leader>ws", M.switch_to_worktree, { desc = "Switch worktree" })
end

-- Auto-setup with defaults if loaded directly
M.setup()
